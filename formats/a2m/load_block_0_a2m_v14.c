...
static int8_t load_block_0_a2m_v14 (uint8_t version, tFIXED_SONGDATA *song,
                                    size_t packed_size,
                                    FILE *f,
                                    progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  for (unsigned i = 0; i < lengthof (raw_data->pat_names, [0]); i++)
    if (raw_data->pat_names[i].length > sizeof (raw_data->pat_names[0].data)) goto _err_format;
  if (   (uint16_LE (raw_data->pat_length) > PATTERN_LEN_A2M_V9)
      || (raw_data->num_channels == 0)
      || (raw_data->num_channels > CHANNELS_MAX_A2M_V9))
    goto _err_format;

  get_song_name_a2m_v1 (song, &raw_data->song_name);
  get_composer_a2m_v1 (song, &raw_data->composer);

  get_ins_names_a2m_v9 (song, raw_data->ins_names, lengthof (raw_data->ins_names, [0]));

  for (unsigned i = 0; i < lengthof (raw_data->ins_data, [0]); i++)
    get_ins_data_a2m_v9 (&song->instr_data[i], &raw_data->ins_data[i]);
  ...
  for (unsigned i = 0; i < lengthof (raw_data->ins_macros, [0]); i++)
    get_ins_macro_a2m_v9 (&song->instr_macros[i], &raw_data->ins_macros[i]);
  for (unsigned i = lengthof (raw_data->ins_macros, [0]); i < INSTRUMENTS_MAX; i++)
    memset (&song->instr_macros[i], 0, sizeof (song->instr_macros[0]));

  for (unsigned i = 0; i < lengthof (raw_data->macros, [0]); i++)
    get_macro_a2m_v9 (&song->macro_table[i], &raw_data->macros[i]);
  for (unsigned i = lengthof (raw_data->macros, [0]); i < INSTRUMENTS_MAX; i++)
    memset (&song->macro_table[i], 0, sizeof (song->macro_table[0]));

  get_pat_order_a2m_v1 (song, &raw_data->pat_order);

  song->tempo = raw_data->tempo;
  song->speed = raw_data->speed;
  get_flags_a2m_v9 (song, &raw_data->flags);
  song->patt_len = uint16_LE (raw_data->pat_length);
  song->nm_tracks = raw_data->num_channels;
  song->macro_speedup = uint16_LE (raw_data->macro_speedup);
  get_chan_4op_flags_a2m_v10 (song, &raw_data->chan_4op_flags);
  get_chan_flags_a2m_v13 (song, &raw_data->chan_flags);

  get_pat_names_a2m_v11 (song, raw_data->pat_names, lengthof (raw_data->pat_names, [0]));

  for (unsigned i = 0; i < lengthof (raw_data->dis_fmreg_col, [0]); i++)
    get_dis_fmreg_col_a2m_v11 (&song->dis_fmreg_col[i], &raw_data->dis_fmreg_col[i]);
  for (unsigned i = lengthof (raw_data->dis_fmreg_col, [0]); i < INSTRUMENTS_MAX; i++)
    memset (&song->dis_fmreg_col[i], 0, sizeof (song->dis_fmreg_col[0]));

  get_ins_4op_data_a2m_v12 (song, &raw_data->ins_4op_data);

  song->bpm_data.rows_per_beat = raw_data->rows_per_beat;
  song->bpm_data.tempo_finetune = int16_LE (raw_data->tempo_finetune);
