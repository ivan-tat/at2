...
static int8_t pack_a2m (uint8_t version,
                        ...
                        progress_callback_t *progress, char **error)
{
  ...
  switch (version)
  {
    ...
    case 13:
    case 14:
      actual_size = LZH_compress_ultra (src, dst, src_size, progress);
      if (actual_size > dst_limit) goto _err_fatal; // FIXME: missing checks in `LZH_compress_ultra()'
      status = 0;
      break;
