...
static int8_t load_block_0_a2m_v5 (uint8_t version, tFIXED_SONGDATA *song,
                                   size_t packed_size,
                                   FILE *f,
                                   progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  block_0_a2m_v5_t *raw_data = NULL;
  ...
  for (unsigned i = 0; i < lengthof (raw_data->ins_data, [0]); i++)
    get_ins_data_a2m_v5 (&song->instr_data[i], &raw_data->ins_data[i]);
  ...
  get_flags_a2m_v5 (song, &raw_data->flags);
