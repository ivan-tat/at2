...
static int8_t load_patterns_a2m_v5 (uint8_t version, tFIXED_SONGDATA *song,
                                    ...
                                    progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  pat_data_a2m_v5_t (*raw_data)[8] = NULL;
  ...
  num_blocks = ((num_patterns > PATTERNS_MAX_A2M_V1 ? PATTERNS_MAX_A2M_V1 : num_patterns)
                + lengthof ((*raw_data), [0]) - 1) / lengthof ((*raw_data), [0]); // 0..8
  ...
  for (unsigned i = 0; i < num_blocks; i++)
  {
    ...
    if (packed_size != 0)
    {
      ...
      for (unsigned j = 0; j < count; j++)
      {
        ...
        if (idx < max_patterns)
          get_pat_data_a2m_v5 (get_pattern_data (song, idx), &(*raw_data)[j]);
        else
        {
          limit_exceeded = true;
          break;
