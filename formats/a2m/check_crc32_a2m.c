...
// In:
//   * `valid_crc32': original CRC-32 value from file.
//   * `data_first':
//     `false' - calculate CRC-32 value first for block sizes, then for data;
//     `true'  - calculate CRC-32 value first for data, then for block sizes.
...
static int8_t check_crc32_a2m (uint32_t valid_crc32, bool data_first,
                               ...
                               char **error)
{
  ...
  if (!data_first)
    _block_sizes_crc32 (&crc32, width, width_crc, block_sizes, capacity);
  ...
  for (unsigned i = 0; i < count; i++)
  {
    ...
    if (size != 0)
    {
      ...
      crc32 = Update32 (data, size, crc32);
    }
  }
  ...
  if (crc32 != uint32_LE (valid_crc32)) goto _err_checksum;
