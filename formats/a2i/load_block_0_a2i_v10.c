...
static int8_t load_block_0_a2i_v10 (uint8_t version, temp_instrument_t *dst,
                                    size_t packed_size,
                                    ...
                                    progress_callback_t *progress, char **error)
{
  ...
  raw_limit = (sizeof (ins_data_a2i_v9_t) + INSTRUMENT_NAME_MAX_A2I_V9) * 2;
  ...
  set_mem_stream (&stream, raw_data, raw_size);
  ...
  if (stream.curptr < stream.endptr)  // more data present => 4-op instrument
  {
    ...
    if (swap_ins)
    {
      memcpy (&dst->ins2.fm, &dst->ins1.fm, sizeof (dst->ins2.fm)); // copy to 4-op 2/2
      memcpy (dst->ins2.name, dst->ins1.name, sizeof (dst->ins2.name)); // copy to 4-op 2/2
      dst_ins = &dst->ins1; // 4-op 1/2
    }
    else
      dst_ins = &dst->ins2; // 4-op 2/2

    if (get_ins_a2i_v9 (dst_ins,
                        &stream,
                        &result_error)) goto _exit;
  }
  else
