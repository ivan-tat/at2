...
static int8_t save_block_0_a2w_v3 (uint8_t version, FILE *f,
                                   ...
                                   progress_callback_t *progress, char **error)
{
  ...
  block_0_a2w_v3_t *raw_data = NULL;
  ...
  put_ins_names_a2w_v9 (raw_data->ins_names, song, lengthof (raw_data->ins_names, [0]));

  for (unsigned i = 0; i < lengthof (raw_data->ins_data, [0]); i++)
    put_ins_data_a2w_v1 (&raw_data->ins_data[i], song, i);

  for (unsigned i = 0; i < lengthof (raw_data->ins_macros, [0]); i++)
    put_ins_macro_a2w_v1 (&raw_data->ins_macros[i], song, i);

  put_ins_4op_data_a2w_v3 (&raw_data->ins_4op_data, song);
