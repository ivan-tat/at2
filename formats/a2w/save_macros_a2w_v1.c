...
static int8_t save_macros_a2w_v1 (uint8_t version, FILE *f,
                                  ...
                                  progress_callback_t *progress, char **error)
{
  ...
  macro_a2w_v1_t (*raw_data)[INSTRUMENTS_MAX_A2W_V1] = NULL;
  void *packed_data = NULL;
  size_t raw_size, packed_limit, packed_size;

  if (Empty (song->macro_table, sizeof (song->macro_table[0]) * lengthof ((*raw_data), [0])))
  {
    *block_size = 0;
    return 0;
  }
  ...
  for (unsigned i = 0; i < lengthof ((*raw_data), [0]); i++)
    put_macro_a2w_v1 (&(*raw_data)[i], song, i);
