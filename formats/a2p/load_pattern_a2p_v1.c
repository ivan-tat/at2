...
static int8_t load_pattern_a2p_v1 (uint8_t version, tFIXED_SONGDATA *song, unsigned idx, String *def_name,
                                   size_t packed_size,
                                   FILE *f,
                                   progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  if ((pattern2use != UINT8_NULL) && (_patts_marked () != 0))
  {
    for (unsigned pat = 0; pat < max_patterns; pat++)
      ...
      {
        get_pat_data_a2p_v1 (raw_pat_data, raw_data, &adsr_carrier);
  ...
  //replace_old_adsr_a2p_v1 (song, max_patterns, &adsr_carrier);
