...
/*****************************************************************************
  ...
  AdLib Tracker II pattern file saver
  Supported file format version: 11
...
int8_t save_pattern_a2p (const String *_fname,
...
#pragma pack(push, 1)
  ...
  {
    header_a2p_t    main;
    header_a2p_v9_t data;
  } header;
#pragma pack(pop)
  char fname[255+1];

  DBG_ENTER ("save_pattern_a2p");
  ...
  memcpy (header.main.id, id_a2p, sizeof (header.main.id));
  header.main.crc32 = CRC32_INITVAL;  // we'll update it in the end
  header.main.version = 11;
  ...
  if (save_pattern_a2p_v10 (header.main.version, f,
  ...
  _block_sizes_crc32 (&header.main.crc32,
                      32, 16, &header.data.block_size, 1);
...
_exit:
  ...
  DBG_LEAVE (); //EXIT //save_pattern_a2p
  return result;
