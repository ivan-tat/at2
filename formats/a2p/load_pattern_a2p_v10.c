...
static int8_t load_pattern_a2p_v10 (uint8_t version, tFIXED_SONGDATA *song, unsigned idx, String *def_name,
                                    size_t packed_size,
                                    FILE *f,
                                    progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  if ((pattern2use != UINT8_NULL) && (_patts_marked () != 0))
  {
    for (unsigned pat = 0; pat < max_patterns; pat++)
      ...
      {
        get_pat_data_a2p_v9 (raw_pat_data, &raw_data->pat_data);
        set_pattern_data (song, pat, raw_pat_data);
        set_pattern_name (song, pat, (String *)&raw_data->pat_name, def_name);
        result_state = 2;
      }
  }
  else
  {
    if (idx < max_patterns)
    {
      ...
      set_pattern_name (song, idx, (String *)&raw_data->pat_name, def_name);
      result_state = 2;
    }
    else
      limit_exceeded = true;
