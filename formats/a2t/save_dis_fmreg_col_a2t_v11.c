...
static int8_t save_dis_fmreg_col_a2t_v11 (uint8_t version, FILE *f,
                                          ...
                                          progress_callback_t *progress, char **error)
{
  ...
  if (   (num_instruments == 0)
      || Empty (song->dis_fmreg_col, sizeof (song->dis_fmreg_col[0]) * num_instruments))
  {
    *block_size = 0;
    return 0;
  }
  ...
  for (unsigned i = 0; i < num_instruments; i++)
    put_dis_fmreg_col_a2t_v11 (&(*raw_data)[i], song, i);
