...
static int8_t load_ins_data_a2t_v5 (uint8_t version, tFIXED_SONGDATA *song,
                                    size_t packed_size,
                                    FILE *f,
                                    progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  ins_data_a2t_v1_t (*raw_data)[INSTRUMENTS_MAX_A2T_V1] = NULL;
  ...
  for (unsigned i = 0; i < count; i++)
    get_ins_data_a2t_v5 (&song->instr_data[i], &(*raw_data)[i]);
