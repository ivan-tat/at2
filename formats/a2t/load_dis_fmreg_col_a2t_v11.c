...
static int8_t load_dis_fmreg_col_a2t_v11 (uint8_t version, tFIXED_SONGDATA *song,
                                          size_t packed_size,
                                          FILE *f,
                                          progress_callback_t *progress, uint8_t *state, char **error)
{
  ...
  dis_fmreg_col_a2t_v11_t (*raw_data)[INSTRUMENTS_MAX_A2T_V9] = NULL;
  ...
  count = raw_size / sizeof ((*raw_data)[0]);
  if (sizeof ((*raw_data)[0]) * count != raw_size) goto _err_format;

  for (unsigned i = 0; i < count; i++)
    get_dis_fmreg_col_a2t_v11 (&song->dis_fmreg_col[i], &(*raw_data)[i]);
