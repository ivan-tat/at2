...
static int8_t save_macros_a2t_v9 (uint8_t version, FILE *f,
                                  ...
                                  progress_callback_t *progress, char **error)
{
  ...
  if (   (num_instruments == 0)
      || Empty (song->macro_table, sizeof (song->macro_table[0]) * num_instruments))
  {
    *block_size = 0;
    return 0;
  }
  ...
  for (unsigned i = 0; i < num_instruments; i++)
    put_macro_a2t_v9 (&(*raw_data)[i], song, i);
