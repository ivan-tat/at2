...
static int8_t save_block_0_a2f_v2 (uint8_t version, FILE *f,
                                   uint32_t *crc32, uint16_t *block_size,
                                   ...
                                   progress_callback_t *progress, char **error)
{
  ...
  if (pack_a2f (version, packed_data, packed_limit, &packed_size, raw_data, raw_size,
  ...
  *crc32 = Update32 (block_size, /*sizeof (*block_size)*/ 1, *crc32); // it's not a bug - it's a feature
