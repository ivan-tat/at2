...
static void import_fin_instrument_alt (tADTRACK2_INS *ins, const void *data)
{
  DBG_ENTER ("import_fin_instrument_alt");
  ...
  ins->fm_data.WAVEFORM_modulator  = ((uint8_t *)data)[8]  & 7;
  ins->fm_data.WAVEFORM_carrier    = ((uint8_t *)data)[9]  & 7;
  ins->fm_data.FEEDBACK_FM         = ((uint8_t *)data)[10] & 0x0F;

  ins->panning = 0;
  ins->fine_tune = 0;

  DBG_LEAVE (); //EXIT //import_fin_instrument_alt
}
