...
#define IN_TEXT_WIDTH  90
#define IN_TEXT_HEIGHT 36
#define IN_WIDTH  (FONT_WIDTH * IN_TEXT_WIDTH)   // =720
#define IN_HEIGHT (FONT_HEIGHT * IN_TEXT_HEIGHT) // =576
#define OUT_WIDTH  800 // 100 characters
#define OUT_HEIGHT 600 // 37.5 characters

static void draw_VESA_screen_800x600_2 (void)
{
  const uint8_t *font = font8x16; // input font
  const uint8_t *s = screen_ptr; // input text screen
  uint8_t *p     = &_FrameBuffer_mirror[(OUT_HEIGHT - IN_HEIGHT) / 2 * OUT_WIDTH + (OUT_WIDTH - IN_WIDTH) / 2]; // output buffer
  uint8_t *p_end = &_FrameBuffer_mirror[(OUT_HEIGHT - IN_HEIGHT) / 2 * OUT_WIDTH + (OUT_WIDTH - IN_WIDTH) / 2
                                        + (IN_HEIGHT - 1) * OUT_WIDTH + IN_WIDTH]; // output buffer end
  uint32_t skip = virtual_screen__first_row; // output buffer start offset
  ...
  for (uint8_t r = MAX_ROWS; r; r--) // input text screen rows left
  {
    ...
    for (uint8_t fr = FONT_HEIGHT; fr; fr--) // character's rows left
    {
      ...
      for (uint8_t c = IN_TEXT_WIDTH; c; c--) // input text screen columns left
      {
        ...
        else
        {
          ...
          else
          {
            ...
            uint8_t bit = 0x80 >> skip; // skip pixels
      ...
      if (skip != 0)
        skip -= OUT_WIDTH - IN_WIDTH; // next (decrease) output buffer skip offset
      else
        p += OUT_WIDTH - IN_WIDTH; // next output buffer row
